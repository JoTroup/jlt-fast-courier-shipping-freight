...
$orderStatus = isset($_GET['fc_order_status']) ? sanitize_text_field($_GET['fc_order_status']) : $fc_order_status['unprocessed']['key'];
$paymentMethodConfigured = 1;
...
<div class="row">
    <div class="col-sm-3">
        <button class="btn btn-primary mt-2 mx-2" data-action="book-orders" onclick="toggleStatusModal(this)">Book Selected Orders</button>
    </div>
    <div class="col-sm-3">
        <button class="btn btn-primary mt-2 mx-2" data-action="hold-orders" onclick="toggleStatusModal(this)">Hold Selected Orders</button>
    </div>
</div>
    ...
    <table class="table fc-table">
        ...
        <tbody>
            ...
            foreach ($orders as $order) {
                ...
                $shippingAddress1 = ($order->get_shipping_address_1()) ? $order->get_shipping_address_1() : $order->get_billing_address_1();
                $shippingAddress2 = ($order->get_shipping_address_2()) ? $order->get_shipping_address_2() : $order->get_billing_address_2();
            ...
            ?>
                <tr>
                    <td>
                        <?php
                        if (count($errors) == 0 && $order->get_meta('fc_status') == $fc_order_status['unprocessed']['key']) {
                        ?>
                            <input type="checkbox" class="fc-new-order-checkbox" value="<?php echo esc_attr($order->get_id()) ?>" name="orders[]">
                        <?php
                        }
                        ?>
                    </td>
                    ...
                    <td><?php echo html_entity_decode(esc_html(($order->get_formatted_shipping_address()) ? $order->get_formatted_shipping_address() : $order->get_formatted_billing_address())) ?></td>
                    <?php if (count($errors)) { ?>
                        <td><span class="btn status-chip error position-relative" title="Click here to see Error List" onclick='showOrderErrors(<?php echo json_encode($errors) ?>)'>
                                Invalid Data <i class="fa-solid fa-circle-info"></i>
                            </span></td>
                    <?php } else { ?>
                        <td><?php echo fc_order_status_chip($order->get_meta('fc_status')) ?></td>
                    <?php } ?>
                    <td>$<?php echo esc_html($order->get_total()) ?></td>
                    <td><?php echo esc_html(is_array($packs) ? esc_html(count($packs)) : '--') ?></td>
                    <td>
                        <?php
                        ...
                        ?>
                                ...
                                <?php echo esc_html(isset($quote_item['data']['priceIncludingGst']) ? '$' . $quote_item['data']['priceIncludingGst'] : '--') ?>
                                (<?php echo esc_html(isset($quote_item['data']['courierName']) ? $quote_item['data']['courierName'] : '--') ?>)<br>
                            ...
                            <?php echo esc_html(isset($quote['data']['priceIncludingGst']) ? '$' . $quote['data']['priceIncludingGst'] : '--') ?>
                            (<?php echo esc_html(isset($quote['data']['courierName']) ? $quote['data']['courierName'] : '--') ?>)<br>
                    ...
                    <td>
                        <a class="btn btn-warning" href="<?php echo esc_html($order->get_edit_order_url()) ?>"><i class="fa-solid fa-pen-to-square"></i></a><br>
                    </td>
                </tr>
            <?php
            }
...
$date = date('Y-m-d');
$maxDate = date('Y-m-d', strtotime('+6days', strtotime($date)));
...
<div class="modal process-order-modal">
            ...
            <div class="modal-body">
                <form action="" name="selectedOrders">
                    <div class="form-group">
                        <label class="label-required">Collection Date</label>
...
<div class="modal hold-order-modal">
    <div class="modal-dailog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="order-modal-title">Hold</h3>
            </div>
            <div class="modal-footer">
                <button class="btn btn-light" onclick="toggleStatusModal(this)">Close</button>
                <button class="btn btn-primary ml-2" onclick="holdOrders()">Submit</button>
            </div>
        </div>
    </div>
</div>

<div class="modal" id="modalOrderErrors">
    <div class="modal-dailog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="order-modal-title">Fix following details to process this order</h3>
            </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-light" onclick="closeOrderErrors()">Close</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function toggleStatusModal(_this) {
        const paymentMethodConfigured = <?php echo $paymentMethodConfigured ?>;
        if (!paymentMethodConfigured) {
            Swal.fire('', 'Please configure the payment method', 'error');
            return;
        }
        ...
        if (action == 'hold-orders') {
            $('.hold-order-modal').toggleClass('show d-flex align-items-center justify-content-center');
        } else if (action == 'book-orders') {
            $('.process-order-modal').toggleClass('show d-flex align-items-center justify-content-center');
        } else if (action == 'close-date-modal') {
            let dPicker = document.getElementById('collection_date');
            dPicker.value = '';
            $('.modal').removeClass('show d-flex align-items-center justify-content-center');
        } else {
            $('.modal').removeClass('show d-flex align-items-center justify-content-center');
        }
    ...
    function processOrders() {
        ...
        const params = `action=post_process_orders&${orders}`;
    ...
    function holdOrders() {
        ...
        const orders = $("[name='selectedOrders']").serialize();
        const params = `action=post_hold_orders&${orders}`;
        $.post(ajaxurl, params, function(result) {
            ...
            if (result.status == 200) {
                ...
                Swal.fire('', 'Order holding request submitted', 'success').then(function() {
                    location.reload();
                });;
            } else {
                Swal.fire('', result.message, 'error').then(() => {
    ...
    function showOrderErrors(errors) {
        ...
        $.each(errors, function(index, value) {
            $('#errorList').append('<p>' + (index + 1) + '. ' + value + '</p>');
        });

        $('#modalOrderErrors').toggleClass('show d-flex align-items-center justify-content-center');
    }

    function closeOrderErrors(order_id) {
        $('#modalOrderErrors').toggleClass('show d-flex align-items-center justify-content-center');
    }
</script>